...
// ── API Proxy ──
// Route all /api/* fetch calls directly to the orchestrator on port 8080.
// No Caddy proxy needed for local development.
(function() {
    const ORCHESTRATOR_API = "http://localhost:8080";
    const origFetch = globalThis.fetch;
    globalThis.fetch = function(url, opts) {
        if (typeof url === "string" && url.startsWith("/api/")) {
            url = ORCHESTRATOR_API + url;
        }
...
class VoxVoiceManager extends Application {
            ...
            html += `
                <li style="background: rgba(255,255,255,0.03); margin-bottom: 10px; border-radius: 4px; padding: 12px; border-left: 3px solid #ff6400; box-shadow: 0 2px 5px rgba(0,0,0,0.3);">
                    ...
                    <div style="display:flex;gap:4px;margin-top:4px;align-items:center;">
                        <input type="text" class="vox-actor-id-input" value="${entry.id}" style="flex:1;background:#222;color:#aaa;border:1px solid #444;border-radius:4px;padding:1px 4px;font-size:9px;font-family:monospace;height:18px;">
                        <button class="vox-relink-actor" data-old-id="${entry.id}" style="height:16px;line-height:1;font-size:8px;padding:0 3px;background:#333;color:#ffaa22;border:1px solid #887700;border-radius:3px;cursor:pointer;"><i class="fas fa-link"></i></button>
                    </div>
                </li>
            `;
        }
    ...
    activateListeners(html) {
        ...
        $(html).find('.vox-clone-mic').click(async (ev) => {
            ...
            const CLONE_SCRIPT =
`Welcome, friend. It's good to see you.    (neutral, warm)
I can't believe this is happening! This is incredible news!    (happy, excited)
Why would you do this? After everything we've been through...    (sad, hurt)
Enough! This ends now. You will not take another step forward.    (angry, commanding)
Hmm, I wonder what secrets lie beyond that door.    (curious, thoughtful)
Together, we can face whatever comes our way.    (warm, determined)`;

            // Floating overlay — stays open until recording finishes (Foundry Dialog
            // auto-closes on button click, which would hide the script text).
            ...
            document.getElementById("vox-clone-cancel").onclick = () => overlay.remove();
            document.getElementById("vox-clone-start").onclick = async () => {
                const btn = document.getElementById("vox-clone-start");
                if (btn.disabled) return;
                btn.disabled = true;
                document.getElementById("vox-clone-cancel").disabled = true;
                ...
                for (let i = 5; i > 0; i--) {
                    statusEl.textContent = `⏱️ Recording starts in ${i}...`;
                ...
                recorder.onstop = async () => {
                    ...
                    try {
                        ...
                        if (resp.ok) {
                            ...
                            setTimeout(() => {
                                new Audio(`/api/v1/registry/audio/${actorId}?t=${Date.now()}`).play();
                            }, 500);
                        } else {
                ...
                statusEl.textContent = '🔴 Recording... read the script above.';
                recorder.start();
                setTimeout(() => recorder.stop(), 35000);
            };
        });
