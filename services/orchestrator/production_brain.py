...
async def voice_conversion(request: Request):
    ...
    try:
        ...
        if any(kw in transcription.lower() for kw in tactical_keywords):
            ...
            # Pass transcription as context so the model knows what to look for
            asyncio.create_task(asyncio.to_thread(monster_sight.look_at_battlemap, "/foundry_data/current_view.png", transcription))
