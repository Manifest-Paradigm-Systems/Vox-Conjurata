...
def test_mos_maps_to_both_occupation_keys():
    result = keys_for("mos")
    assert "member_occupation" in result
    assert "military_occupation_code" in result
...
def test_duplicate_mappings_deduped():
    ...
    assert result.count("member_occupation") == 1
    assert result.count("military_occupation_code") == 1
...
def test_expand_dedup():
    # a key reached by two different phrases is yielded exactly once
    result = expand(["mos", "military occupational specialty"])
    assert result.count("member_occupation") == 1
    assert result.count("military_occupation_code") == 1
