...
# Total bytes of inlined context. With a 131k lane the whole package fits, so this
# is what keeps the coder prompt from eating the window the model needs to answer.
INLINE_TOTAL_LIMIT = int(os.getenv("DEVTEAM_INLINE_TOTAL", "48000"))         # bytes of an existing file worth showing the coder
...
# A dotted reference: `documents.coords.pdf_to_fitz`, `documents.signers.signer_from_env`.
# Two or more components, so a bare `os.path` in prose does not drag in the stdlib -- only
# names that could plausibly be files INSIDE this workspace are considered, and each
# candidate is checked against the filesystem before anything is inlined.
_DOTTED = re.compile(r"\b([a-z_][\w]*(?:\.[a-z_][\w]*)+)\b")


def _resolve_dotted(workspace: str, dotted: str) -> str | None:
    """The file a dotted reference lives in, or None.

    `documents.coords.pdf_to_fitz` names a module AND a function, so the rightmost
    component is usually not a file. Drop from the right until one resolves.
    """
    parts = dotted.split(".")
    for cut in range(len(parts) - 1, 0, -1):
        stem = "/".join(parts[:cut])
        for cand in (stem + ".py", stem + "/__init__.py"):
            full = safe_path(workspace, cand)
            if full and os.path.isfile(full):
                return cand
    return None


def context_files(workspace: str, text: str) -> list[str]:
    """Files worth showing the coder, most relevant first.

    Named files come before resolved references: an item that names its own file must not
    lose it to the budget in favour of a module it merely mentions.
    """
    named, referenced = [], []
    for name in sorted(set(re.findall(r"[\w./-]+\.py", text or ""))):
        rel = name.lstrip("./")
        full = safe_path(workspace, rel)
        if full and os.path.isfile(full):
            named.append(rel)
    for dotted in sorted(set(_DOTTED.findall(text or ""))):
        rel = _resolve_dotted(workspace, dotted)
        if rel and rel not in named and rel not in referenced:
            referenced.append(rel)
    return named + referenced


def inline_context(workspace: str, detail: str) -> str:
    """Show the coder the contents of existing files it is likely to need.

    Only files it names or refers to, and only while they fit the budget -- this is prompt
    budget spent to stop the coder inventing an interface that already exists, which is a
    failure it cannot recover from and the verifier catches only as a TypeError.
    """
    chunks, spent = [], 0
    for rel in context_files(workspace, detail or ""):
        full = safe_path(workspace, rel)
        if not full or not os.path.isfile(full):
            continue
        size = os.path.getsize(full)
        if size > INLINE_FILE_LIMIT:
            continue                        # one huge file, skipped whole rather than truncated
        if spent + size > INLINE_TOTAL_LIMIT:
            continue                        # out of budget; keep looking for smaller ones
        try:
...
def run_item(conn, item, log=print) -> str:
    ...
    try:
        ...
        if not rdata:
            # Prose, not a verdict. The director did answer -- it just did not answer in
            # the protocol, which on this lane is the common case. It is a consult that
            # produced nothing, and saying so beats crediting the item with a diagnosis.
            log("    the director replied without a verdict — no diagnosis was produced")
    except Exception as exc:  # noqa: BLE001
        rdata = {}
    ...
    if rdata:
        ...
        # Say which of the two things happened. A blank reason reads as a considered
        # decision to abandon; this was a consult that produced no verdict at all, and
        # the item is blocked on the unrepaired failure above.
        notes = ("NO DIAGNOSIS — the director's reply carried no verdict object (it "
                 "answered in prose). Blocked on the unrepaired failure above, not on "
                 "an architect's decision.")
    if options_text:
        notes = (notes + "\n\n" + options_text).strip()
    jarvis_db.set_item(conn, item_id, status="blocked", evidence=last_error,
                       notes=notes, options=options_json)
...
def use_editor() -> bool:
    if EXECUTOR in ("cerebro", "cerebro-harness", "builtin"):
        return False          # "builtin" kept: the live drop-in still carries that name
    if EXECUTOR == "aider":
        return True
...
def executor_for_attempt(attempt: int) -> bool:
    ...
    the cerebro-harness path on whole-file JSON. Retrying the same one three times spends all
...
BLOCK_RETRIES = int(os.getenv("DEVTEAM_BLOCK_RETRIES", "2"))
_BLOCK_MARK = re.compile(r"\[unblock (\d+)/(\d+)\]")


def _unblock_count(conn, item_id: str) -> int:
    """How many times retry_blocks has already given this item another go.

    Counted from `runs`, and with a boundary, for the two reasons `_attempts` gives:
    a marker kept in the notes is wiped by every re-block, and a counter with no
    boundary is a terminal state that only a human can clear.

    The boundary is the last hand-issued respec, and deliberately NOT a director
    consult. The two boundaries answer different questions. `_attempts` resets on a
    consult because the repair pass that follows either re-specs the item (a new
    specification, which deserves fresh attempts) or blocks it. This counter asks how
    many times the SAME specification has been revived, and a consult does not change
    the specification -- counting it here would re-arm the bound every time it was
    spent, which is the infinite loop this function exists to prevent. A respec IS a
    new specification, so it earns the retries back.
    """
    boundary = conn.execute(
        "SELECT COALESCE(MAX(CAST(ts AS REAL)), 0) t FROM runs"
        " WHERE item_id=? AND role='respec'", (item_id,)).fetchone()["t"]
    return conn.execute(
        "SELECT COUNT(*) FROM runs WHERE item_id=? AND role='unblock'"
        " AND CAST(ts AS REAL) > ?", (item_id, boundary)).fetchone()[0]


def retry_blocks(conn, plan_id: str, log=None) -> int:
    """Give a blocked item another go, a bounded number of times.

    `blocked` marks an operator or harness fault the coder cannot fix, and the classifier
    blocks those on purpose. But the block is the verdict of a model, and on 2026-09-15 two
    of five were false. It is also terminal, and it gates every dependant -- so one wrong
    diagnosis idles a whole plan, which happened three times in one evening.

    So: while the plan has nothing else ready, return ONE blocked item to `pending`, up to
    BLOCK_RETRIES times each. Once its retries are spent it stays blocked, and that is the
    honest answer rather than a loop.
    """
    items = jarvis_db.list_items(conn, plan_id)
    blocked = [i for i in items if i["status"] == "blocked"]
    if not blocked:
        return 0
    # Only when the plan is otherwise stuck. A blocked item is not urgent while other work
    # can move, and retrying it early just spends the lane on the least likely item.
    if jarvis_db.next_ready_item(conn, plan_id) is not None:
        return 0

    for it in blocked:
        # Count from `runs`, NOT from the notes. Every block path rewrites the notes with
        # the director's reason, so a marker kept there is wiped on each re-block and the
        # count silently restarts -- which turned this bound into an infinite loop the
        # first night it ran (LPDOC:W-04, retry "1/2" forever).
        used = _unblock_count(conn, it["id"])
        if used >= BLOCK_RETRIES:
            continue
        jarvis_db.set_item(conn, it["id"], status="pending", options="")
        jarvis_db.log_run(conn, role="unblock", prompt=f"unblock {it['id']}",
                          output=f"retry {used + 1}/{BLOCK_RETRIES}", ok=True, ms=0,
                          plan_id=plan_id, item_id=it["id"], engine="unblock")
        if log:
            log(f"  {it['id']}: blocked -> pending "
                f"(retry {used + 1}/{BLOCK_RETRIES}; it gated {len(items)} item(s) on this plan)")
        return 1        # one per tick: make progress, don't flood the lane
    return 0
...
def do_autopilot(max_items: int = 4, log=print) -> dict:
    ...
    for p in plans:
        ...
        # Before deciding the plan is done: a blocked item is terminal and gates its
        # dependants, so a stale block can idle the whole plan. See retry_blocks.
        retry_blocks(conn, p["id"], log=log)
