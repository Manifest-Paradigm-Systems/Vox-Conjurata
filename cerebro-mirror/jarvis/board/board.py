...
# The devteam worker lives with the brain; the board calls into it to apply an option.
BRAIN_DIR = os.getenv("JARVIS_BRAIN_DIR", "/var/home/admin/jarvis/brain")
...
def collect() -> dict:
    ...
    try:
        ...
        items = [dict(r) for r in conn.execute(
            ...
            " evidence, depends_on, notes, artifact, options FROM work_items")]
    ...
    # Stuck items that have an actionable way out. Jarvis reads these when the owner
    # asks about the team, so they can be talked through and chosen by voice.
    data["options_for"] = []
    for i in items:
        if i["status"] != "blocked" or not i.get("options"):
            continue
        try:
            parsed = json.loads(i["options"])
        except (ValueError, TypeError):
            continue
        if parsed:
            data["options_for"].append({"id": i["id"], "title": i["title"],
                                        "plan": i["plan_id"], "options": parsed})
    data["totals"] = {
...
def api_choose(payload: dict):
    ...
    lines: list[str] = []
    try:
        # devteam lives with the brain, not with the board — the worker owns it.
