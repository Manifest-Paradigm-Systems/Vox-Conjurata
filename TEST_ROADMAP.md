...
- [x] **Workhorse UI v4 Acceptance**: Per-container diagnostics (RAM/CPU/PIDs via podman stats), VRAM progress bar removed, START ALL button wired — `vox-conjurata-webui-v4` PASS (8/8), v2 PASS (9/9), v3 PASS (9/9), smoke PASS (6/6), unit tests 13/13 (2026-08-04). Registry now 5 projects (vox-conjurata, AI-devteam, vox-pdf-importer, tiny11_CAC, LifePacket). WebUI now a systemd user service (`workhorse-webui.service`).
- [x] **Workhorse UI v5 Acceptance**: Dead "LoRA Influence" slider removed; per-container resource settings API (CPU/MEM/RESTART via `podman update`) + RAM peak via cgroup v2 `memory.peak`; multi-project tab membership — `vox-conjurata-webui-v5` PASS (6/6), v4 PASS (8/8), v3 PASS (9/9), v2 PASS (9/9), smoke PASS (6/6), unit tests 16/16 (2026-08-04).
